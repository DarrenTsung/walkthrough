...
fn render_diff_row(
    ...
    if let (Some(lhs), Some(rhs)) = (row.lhs, row.rhs) {
        ...
        if !lhs.changes.is_empty() || !rhs.changes.is_empty() {
            ...
            // Full-line background, with darker token highlights only when:
            // 1. One side's changed region is empty (pure insertion/deletion)
            // 2. The shared prefix+suffix has actual alphabetic content (not
            //    just matching whitespace/punctuation/brackets)
            is_full_line = true;
            let (old_cs, old_ce, new_cs, new_ce) = find_change_bounds(old_line, new_line);
            let old_changed = old_ce - old_cs;
            let new_changed = new_ce - new_cs;
            let shared_has_alpha = old_line[..old_cs].bytes()
                .chain(old_line[old_ce..].bytes())
                .any(|b| b.is_ascii_alphabetic());
            if (old_changed == 0 || new_changed == 0) && shared_has_alpha {
...
fn render_chunks_text(difft: &DifftOutput, chunk_indices: &[usize], line_filter: LineFilter) -> String {
    ...
    for &idx in chunk_indices {
        ...
        // Build unified item list from consolidated difft rows.
        let rows = consolidate_chunk(chunk);
        let mut items: Vec<(u64, &DiffRow)> = Vec::new();
        ...
        items.sort_by(|a, b| {
            a.0.cmp(&b.0).then_with(|| {
                let old_a = a.1.lhs.map(|s| s.line_number);
                let old_b = b.1.lhs.map(|s| s.line_number);
                old_a.cmp(&old_b)
            })
        });
        ...
        if let Some((filter_start, filter_end)) = line_filter {
            items.retain(|&(_, row)| {
                let n = row.rhs.map(|s| s.line_number as usize)
...
fn render_chunks(difft: &DifftOutput, chunk_indices: &[usize], file_path: &str, line_filter: LineFilter, hl: &mut Highlighter, collapse: CollapseMode) -> String {
    ...
    for (chunk_order, &idx) in chunk_indices.iter().enumerate() {
        ...
        // Collect ALL item line numbers before filtering, so
        ...
        let old_ctx_before = old_first.saturating_sub(EXPANDED_CONTEXT_LINES);
        let new_ctx_before = new_first.saturating_sub(EXPANDED_CONTEXT_LINES);
        ...
        if !in_difft_block {
            if line.starts_with("```") && line.chars().filter(|&c| c == '`').count() >= 3 {
                ...
                if let Some(caps) = difft_re.captures(info.trim()) {
                    ...
                    // Validate: file must exist in collected data
                    let difft = data.get(&file).ok_or_else(|| {
                        anyhow::anyhow!(
                            "no collected data for file '{}' referenced in:\n  {}",
                            file, line.trim(),
                        )
                    })?;
                    ...
                    let indices: Vec<usize> = if chunks_spec == "all" {
                        ...
                        for s in chunks_spec.split(',') {
                            let s = s.trim();
                            let idx: usize = s.parse().map_err(|_| {
                                anyhow::anyhow!(
                                    "invalid chunk index '{}' in:\n  {}",
                                    s, line.trim(),
                                )
                            })?;
                            if idx >= difft.chunks.len() {
                                bail!(
                                    "chunk index {} out of range (file '{}' has {} chunks) in:\n  {}",
                                    idx, file, difft.chunks.len(), line.trim(),
                                );
                            }
                            parsed.push(idx);
                        }
                    ...
                    let line_filter: LineFilter = if let Some(m) = caps.get(3) {
                        let s = m.as_str();
                        let parts: Vec<&str> = s.split('-').collect();
                        if parts.len() != 2 {
                            bail!(
                                "invalid lines= format '{}' (expected START-END) in:\n  {}",
                                s, line.trim(),
                            );
                        }
                        let rel_start: usize = parts[0].parse().map_err(|_| {
                            anyhow::anyhow!("invalid lines= start '{}' in:\n  {}", parts[0], line.trim())
                        })?;
                        let rel_end: usize = parts[1].parse().map_err(|_| {
                            anyhow::anyhow!("invalid lines= end '{}' in:\n  {}", parts[1], line.trim())
                        })?;
                        if rel_start == 0 || rel_end == 0 {
                            bail!(
                                "lines= values must be 1-based (got lines={}) in:\n  {}",
                                s, line.trim(),
                            );
                        }
                        if rel_start > rel_end {
                            bail!(
                                "lines= start ({}) is greater than end ({}) in:\n  {}",
                                rel_start, rel_end, line.trim(),
                            );
                        }
                        // Find the earliest and latest new-file lines across the selected chunks
                            ...
                            if let Some(chunk) = difft.chunks.get(ci) {
                                for entry in chunk {
                                    if let Some(rhs) = &entry.rhs {
                                        ...
                                        max_line = max_line.max(rhs.line_number as usize);
                                    }
                                }
                            }
                        }
                        if base == usize::MAX {
                            bail!(
                                "chunks have no new-file lines, cannot apply lines= in:\n  {}",
                                line.trim(),
                            );
                        }
                        let max_rel = max_line - base + 1;
                        if rel_end > max_rel {
                            bail!(
                                "lines= end ({}) exceeds chunk range (max relative line is {}) in:\n  {}",
                                rel_end, max_rel, line.trim(),
                            );
                        }
...
mod tests {
    ...
    /// Old lines 30-32 (1-based) must appear in the rendered output. With
    /// difft's structural matching these were silently excluded because difft
    /// matched them to identical new-side content. With hunk-based chunks,
    /// every line in a hunk range is present.
    #[test]
    fn missing_old_lines_in_difft_chunks() {
        let fixture_path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("test_fixtures/missing-old-lines/services__agentplat__sbox__sboxd__handler__workspace_handler.go.json");
        if !fixture_path.exists() {
            eprintln!("Fixture not found, skipping");
            return;
        }
        ...
        // Render all chunks together to check the full file diff
        let all_indices: Vec<usize> = (0..difft.chunks.len()).collect();
        let mut hl = Highlighter::new();
        let html = render_chunks(&difft, &all_indices, file_path, None, &mut hl, CollapseMode::None);
        let rows = extract_rows(&html);

        let old_lines_in_html: Vec<u64> = rows.iter()
            .filter_map(|(_, lhs, _)| *lhs)
            .collect();

        // Old lines 30, 31, 32 (1-based) are within hunk old_start=29, old_count=4
        // (covers 1-based lines 29-32).
        for expected_line in [30, 31, 32] {
            assert!(
                old_lines_in_html.contains(&expected_line),
                "Old line {} (1-based) missing from rendered output. \
                 Rendered LHS lines: {:?}",
                expected_line, old_lines_in_html,
            );
        }
    }

    /// Helper: render a single paired row from two lines and return the HTML.
    fn render_paired_row(old_line: &str, new_line: &str) -> String {
        let old_lines = vec![old_line.to_string()];
        let new_lines = vec![new_line.to_string()];
        let old_hl = vec![html_escape(old_line)];
        let new_hl = vec![html_escape(new_line)];
        let lhs = LineSide { line_number: 0, changes: vec![] };
        let rhs = LineSide { line_number: 0, changes: vec![] };
        let row = DiffRow { lhs: Some(&lhs), rhs: Some(&rhs) };
        render_diff_row(&row, &old_lines, &new_lines, &old_hl, &new_hl)
    }

    /// Pure insertion within a line (shared alphabetic content, one side's
    /// change is empty) should get token-level highlights.
    #[test]
    fn token_highlight_on_pure_insertion() {
        let html = render_paired_row(
            "    const sandbox = await createSandbox('client-itest')",
            "    const sandbox = await createSandbox('client-itest', WORKLOAD_ENV)",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(html.contains("hl-add"), "pure insertion should have token highlight on new side");
    }

    /// Pure deletion within a line should get token-level highlights.
    #[test]
    fn token_highlight_on_pure_deletion() {
        let html = render_paired_row(
            "    await waitForBootstrap(sboxUrl, timeout)",
            "    await waitForBootstrap(sboxUrl)",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(html.contains("hl-del"), "pure deletion should have token highlight on old side");
    }

    /// Lines that are completely rewritten (both sides have changed content)
    /// should NOT get token-level highlights.
    #[test]
    fn no_token_highlight_on_rewrite() {
        let html = render_paired_row(
            "        let output = Command::new(\"git\")",
            "        let (old_lines, new_lines) = get_file_contents(diff_args, file_path)",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(!html.contains("hl-del"), "rewrite should not have token highlights");
        assert!(!html.contains("hl-add"), "rewrite should not have token highlights");
    }

    /// A line paired against an empty line should NOT get token highlights
    /// even though one side's change region is technically empty. The shared
    /// content has no alphabetic characters.
    #[test]
    fn no_token_highlight_on_empty_pair() {
        let html = render_paired_row(
            "            Vec::new()",
            "",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(!html.contains("hl-del"), "empty pair should not have token highlights");
        assert!(!html.contains("hl-add"), "empty pair should not have token highlights");
    }

    /// Lines sharing only punctuation/whitespace (no alphabetic chars in
    /// common) should NOT get token highlights.
    #[test]
    fn no_token_highlight_on_punctuation_only_match() {
        // Both lines start with whitespace+punctuation but no shared alpha
        let html = render_paired_row(
            "            .env(\"GIT_EXTERNAL_DIFF\", \"difft --display json --color never\")",
            "            .unwrap_or_else(|e| {",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(!html.contains("hl-del"), "punctuation-only match should not have token highlights");
        assert!(!html.contains("hl-add"), "punctuation-only match should not have token highlights");
    }

    /// A small edit to a function name should get token highlights since
    /// the shared prefix has alphabetic content.
    #[test]
    fn token_highlight_on_small_edit() {
        let html = render_paired_row(
            "    let output = Command::new(\"git\")",
            "    let output = Command::new(\"git-foo\")",
        );
        assert!(html.contains("line-paired-full"), "should have full-line background");
        assert!(html.contains("hl-add"), "small edit should have token highlight");
    }
}
